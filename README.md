...
| `fleet subagent run --provider <claude\|codex> [--brief-file <FILE>] --expect <TEXT> [--worktree <ID>] [--mode <MODE>] [--model <MODEL>] [--effort <EFFORT>] [--title <TITLE>] [--eager] [--caller <THREAD>] [--json]` | Start a delegated child thread, reading the brief from the file or stdin; the caller is `--caller` or `FLEET_SESSION`. `--effort` is provider-native free text and may be passed without `--model`, in which case the child keeps that provider's configured default model. | `protocol`, `delegation`, optional `warning` |
