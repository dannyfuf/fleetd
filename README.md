...
| `fleet subagent run --provider <claude\|codex> [--brief-file <FILE>] --expect <TEXT> [--worktree <ID>] [--mode <MODE>] [--model <MODEL>] [--effort <EFFORT>] [--title <TITLE>] [--eager] [--caller <THREAD>] [--env <KEY=VALUE>]... [--json]` | Start a delegated child thread, reading the brief from the file or stdin; the caller is `--caller` or `FLEET_SESSION`. `--effort` is provider-native free text and may be passed without `--model`, in which case the child keeps that provider's configured default model. `--env` is repeatable, is merged under Fleet's own `FLEET_*` variables, is kept across a resume, and refuses a value with no `=`, an empty or repeated key, a `FLEET_`-prefixed key, and `PATH`. The JSON envelope cuts a brief longer than 200 characters and then sets `briefElided: true`; a shorter brief is carried whole and the key is absent. | `protocol`, `delegation`, optional `warning`, `briefElided` when the brief was cut |
...
| `fleet subagent wait <ID> [--timeout <SECONDS>] [--caller <THREAD>] [--json]` | Wait for a delegation, returning the child's report on success. `--timeout` defaults to 540 seconds and has no upper bound; a timeout exits 2 with a still-running line, a terminal record exits 0. The caller is `--caller` or `FLEET_SESSION` and is optional; when it matches the delegation's own caller the result is marked `consumed` and is not delivered again as a user message. The JSON envelope cuts a brief longer than 200 characters and then sets `briefElided: true`; a shorter brief is carried whole and the key is absent. | `protocol`, `delegation`, `briefElided` when the brief was cut |
| `fleet subagent status <ID> [--json]` | Show one delegation: the fixed-field line, the brief, the child's token usage and cost, and the child's report for a terminal delegation, rendered exactly as `wait` renders it. The JSON envelope keeps the brief whole. | `protocol`, `delegation` |
| `fleet subagent list [--caller <THREAD>] [--json]` | List delegations, optionally for one calling thread, one eight-field tab-separated line each: id, status, provider, child, duration, total tokens, cost, delivery. Token and cost show `-` when unknown; both cover the child's own thread only. The JSON envelope cuts every brief longer than 200 characters and then sets `briefElided: true`; a page whose briefs are all shorter is carried whole and the key is absent. | `protocol`, `delegations`, `briefElided` when a brief was cut |
