...
| `fleet subagent list [--caller <THREAD>] [--json]` | List delegations, optionally for one calling thread, one eight-field tab-separated line each: id, status, provider, child, duration, total tokens, cost, delivery. Token and cost show `-` when unknown; both cover the child's own thread only. The JSON envelope truncates each brief to 200 characters and sets `briefElided`. | `protocol`, `delegations`, `briefElided` |
