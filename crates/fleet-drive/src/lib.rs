...
//! keystroke translation. `fleet-lazygit` takes `legacy` alone, so `regex`, the predicate
//! evaluator and the server loop stay out of a binary that drives nothing over a socket.
