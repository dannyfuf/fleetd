...
use crate::{
    actions::{
        board, card_detail, confirm, daemon as daemon_actions, dialog, filter,
        first_run as first_run_actions, fleet, help, hub, jobs, palette, prefix,
        quit_daemon_dialog, quit_dialog, repos, scroll, workspace,
    },
...
impl Shell {
    ...
    fn board_go_board(&mut self, _: &board::GoBoard, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::go_board(&self.state, &self.bridge, cx);
    }

    fn board_prev_column(&mut self, _: &board::PrevColumn, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::prev_column(&self.state, &self.bridge, cx);
    }

    fn board_next_column(&mut self, _: &board::NextColumn, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::next_column(&self.state, &self.bridge, cx);
    }

    fn board_next_card(&mut self, _: &board::NextCard, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::next_card(&self.state, &self.bridge, cx);
    }

    fn board_prev_card(&mut self, _: &board::PrevCard, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::prev_card(&self.state, &self.bridge, cx);
    }

    fn board_open_card(&mut self, _: &board::OpenCard, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::open_card(&self.state, &self.bridge, cx);
    }

    fn board_new_card(&mut self, _: &board::NewCard, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::new_card(&self.state, &self.bridge, cx);
    }

    fn board_pick_status(&mut self, _: &board::PickStatus, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::pick_status(&self.state, &self.bridge, cx);
    }

    fn board_pick_priority(
        &mut self,
    ...
    fn board_pick_labels(&mut self, _: &board::PickLabels, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::pick_labels(&self.state, &self.bridge, cx);
    }

    fn board_pick_estimate(
        &mut self,
    ...
    fn board_sync(&mut self, _: &board::Sync, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::sync(&self.state, &self.bridge, false, cx);
    }

    fn board_full_sync(&mut self, _: &board::FullSync, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::sync(&self.state, &self.bridge, true, cx);
    }

    fn board_open_remote(&mut self, _: &board::OpenRemote, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::open_remote(&self.state, &self.bridge, cx);
    }

    fn board_delete_card(&mut self, _: &board::DeleteCard, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::delete_card(&self.state, &self.bridge, cx);
    }

    fn board_settings(&mut self, _: &board::Settings, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::settings(&self.state, &self.bridge, cx);
    }

    fn board_reload(&mut self, _: &board::Reload, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::reload(&self.state, &self.bridge, cx);
    }

    fn board_filter(&mut self, _: &board::Filter, _: &mut Window, cx: &mut Context<Self>) {
        crate::screens::board::filter(&self.state, &self.bridge, cx);
    }

    fn card_detail_close(
        &mut self,
        _: &card_detail::Close,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    fn card_detail_edit_description(
        &mut self,
    ...
    fn card_detail_create_worktree(
        &mut self,
    ...
    fn card_detail_save(&mut self, _: &card_detail::Save, _: &mut Window, cx: &mut Context<Self>) {
...
impl Shell {
    ...
    fn with_actions(
        ...
        .on_action(cx.listener(Self::board_go_board))
        .on_action(cx.listener(Self::board_prev_column))
        .on_action(cx.listener(Self::board_next_column))
        .on_action(cx.listener(Self::board_next_card))
        .on_action(cx.listener(Self::board_prev_card))
        .on_action(cx.listener(Self::board_open_card))
        .on_action(cx.listener(Self::board_new_card))
        .on_action(cx.listener(Self::board_pick_status))
        .on_action(cx.listener(Self::board_pick_priority))
        .on_action(cx.listener(Self::board_pick_assignee))
        .on_action(cx.listener(Self::board_pick_labels))
        .on_action(cx.listener(Self::board_pick_estimate))
        .on_action(cx.listener(Self::board_move_prev_column))
        .on_action(cx.listener(Self::board_move_next_column))
        .on_action(cx.listener(Self::board_create_worktree))
        .on_action(cx.listener(Self::board_open_worktree))
        .on_action(cx.listener(Self::board_sync))
        .on_action(cx.listener(Self::board_full_sync))
        .on_action(cx.listener(Self::board_open_remote))
        .on_action(cx.listener(Self::board_delete_card))
        .on_action(cx.listener(Self::board_settings))
        .on_action(cx.listener(Self::board_reload))
        .on_action(cx.listener(Self::board_filter))
        .on_action(cx.listener(Self::card_detail_close))
        .on_action(cx.listener(Self::card_detail_edit_title))
        .on_action(cx.listener(Self::card_detail_edit_description))
        .on_action(cx.listener(Self::card_detail_add_comment))
        .on_action(cx.listener(Self::card_detail_next_property))
        .on_action(cx.listener(Self::card_detail_prev_property))
        .on_action(cx.listener(Self::card_detail_edit_property))
        .on_action(cx.listener(Self::card_detail_create_worktree))
        .on_action(cx.listener(Self::card_detail_open_remote))
        .on_action(cx.listener(Self::card_detail_keep_local))
        .on_action(cx.listener(Self::card_detail_take_remote))
        .on_action(cx.listener(Self::card_detail_save))
