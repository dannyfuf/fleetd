...
async fn run(
    ...
    while let Ok(incoming) = channel.requests.recv().await {
        ...
        if quit {
            if let Err(error) = close_harness_window(cx, harness.headless) {
...
/// Closes the driven window after GPUI releases its focused platform input handler.
fn close_harness_window(cx: &mut AsyncWindowContext, headless: bool) -> gpui::Result<()> {
    cx.update(|window, _| {
        if headless {
            // The headless platform has no frame source and installs no platform input handler.
            window.remove_window();
        } else {
            // A platform input handler retains the focused entity until an unfocused frame
            // replaces it. Closing in this update leaves that entity alive for leak detection.
