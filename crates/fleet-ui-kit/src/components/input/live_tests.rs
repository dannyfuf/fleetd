...
pub(crate) fn test_bindings() -> Vec<KeyBinding> {
    // One table for the whole workspace: the editor's own rows come from the kit, and the host
    // below adds the container key a surface around an editor is expected to keep.
    let mut bindings = text_input::default_bindings();
    bindings.push(KeyBinding::new(
        "enter",
        ParentEnter,
        Some("TextInputTestHost"),
    ));
    bindings
}
